...
void CL_APAIR::F_WCSA_APAIR_WCS(){
    ...
    if(STDOUT==1||STDOUT==2)cout << "--- WCS_TANSIP : CALCULATING GLOBAL WCS : GET MAGROT--" << endl;
...
CRVAL[1]=CENTER_RADEC[1];
    }else if(strcmp(CRPIXMODE,"CRPIX")==0){
        F_WCSA_APAIR_CALCCRVAL();
    }else if(strcmp(CRPIXMODE,"CRVAL")==0){
...
//--------------------------------------------------
    ...
    if(PAIR[NUM].FLAG == 1){
	      ...
	      if(FUNCTION==0){
            ...
            dx[i][FNUM][2]=PAIR[NUM].CAMERA_MAGROT[i];
        }else if(FUNCTION==7){
    ...
    for(NUM=0;NUM<ALLREFNUM;NUM++){
        ...
        for(i=0;i<2;i++)
	      if(FUNCTION==0){
            ...
            dx[i][CID][CNUM[CID]][2]=PAIR[NUM].CAMERA_MAGROT[i];
        }else if(FUNCTION==7){
...
//--------------------------------------------------
    ...
    for(NUM=0;NUM<ALLREFNUM;NUM++)
    if(PAIR[NUM].FLAG == 1){
        data[0][NUM]=PAIR[NUM].X_IM_PIXEL[0]-(F_CALCVALUE(SIP_ORDER,TSIP_AB[0],PAIR[NUM].X_CRPIX)+PAIR[NUM].X_CRPIX[0]);
        data[1][NUM]=PAIR[NUM].X_IM_PIXEL[1]-(F_CALCVALUE(SIP_ORDER,TSIP_AB[1],PAIR[NUM].X_CRPIX)+PAIR[NUM].X_CRPIX[1]);
        FNUM++;
    }
...
//--------------------------------------------------
    ...
    for(NUM=0;NUM<ALLREFNUM;NUM++)
    if(PAIR[NUM].FLAG == 1){
        data[0][NUM]=PAIR[NUM].X_CRPIX[0]-(F_CALCVALUE(SIP_P_ORDER,TSIP_ABP[0],PAIR[NUM].X_IM_PIXEL)+PAIR[NUM].X_IM_PIXEL[0]);
        data[1][NUM]=PAIR[NUM].X_CRPIX[1]-(F_CALCVALUE(SIP_P_ORDER,TSIP_ABP[1],PAIR[NUM].X_IM_PIXEL)+PAIR[NUM].X_IM_PIXEL[1]);
        FNUM++;
    }
...
void CL_APAIR::F_WCSA_APAIR_CCDPOSITIONS_XY_SETAVERAGE(){
    int CID;
    for(CID=0;CID<CCDNUM;CID++){
    GPOS[CID][0]-=GPOS_AVE[0];
    GPOS[CID][1]-=GPOS_AVE[1];
    }
}
//ETC

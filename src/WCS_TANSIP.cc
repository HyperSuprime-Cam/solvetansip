...
void    F_WSC_eachCHIP(CL_APROP,CL_CPROP*,CL_CSIP*);
...
cout << "--- F_WCS_TANSIP:FIT ---" << endl;// check
    ...
    F_WSC_eachCHIP(APROP,CPROP,CSIP);
...
cout << "A_DISP      : " << CSIP[APROP.CCDNUM].SIP_AB_DISP[0] << endl;// check
cout << "B_DISP      : " << CSIP[APROP.CCDNUM].SIP_AB_DISP[1] << endl;// check
...
/*
afwImage::TanWcs::Ptr    F_WCS_TANSIP(vector< vector<afwdetect::SourceMatch> > const &matchlist,lsst::pex::policy::Policy::Ptr &APROPPolicy,lsst::afw::cameraGeom::Camera::Ptr &camera){
int CID;
vector< vector<afwdetect::SourceMatch> > matchlist_VV;
    CL_APROP  APROP;
    CL_CPROP *CPROP;
    CL_PAIR   PAIR;
    CL_CSIP  *CSIP;
...
//output format --------------------------------------------------
    afwImage::TanWcs::Ptr resultWcs = F_CLWCSfromCSIP(&CSIP[APROP.CCDNUM]);
